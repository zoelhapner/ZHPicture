    ...
    <div class="container-xl">
        <div class="row align-items-center">
            <div class="col d-flex align-items-center">
                <a href="{{ route('workers.index') }}" class="btn btn-dark d-flex align-items-center" style="margin-left: 30px;">
                    <i class="ti ti-arrow-left"></i>
                </a>
                
                    <h2 class="page-title mb-0">Tambah Data Tukang</h2>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body px-5 py-4">
                <form action="{{ route('workers.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="section-block mb-5">
                        <h3 class="fw-semibold mb-3 border-bottom pb-2">💼 Data Tukang</h3>
                        <div class="row g-4">
                            <div class="col-md-4">
                                <label class="form-label">ID Tukang</label>
                                <input type="text" id="worker_id" name="worker_id" class="form-control" readonly>
                                @error('worker_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-10">
                                ...
                                <select class="form-select select2" name="role[]" multiple>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const worker_idField = document.getElementById('worker_id'); // ubah nama biar sesuai

        // Pastikan elemen ada dulu
        if (!worker_idField) return;

        fetch('{{ route('workers.generateWorkerId') }}', {
            headers: {
        ...
        .then(data => {
            if (data.worker_id) {
                worker_idField.value = data.worker_id; // variabel sekarang valid
            }
        })
        .catch(error => console.error('❌ Error fetching worker_id:', error));
    });
</script>
