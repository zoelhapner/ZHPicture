        ...
        <div class="row align-items-center">
            <div class="col d-flex align-items-center">
                <a href="{{ route('suppliers.index') }}" class="btn btn-dark d-flex align-items-center">
                    ...
                    <h2 class="page-title mb-0">Tambah Data Supplier</h2>
                    ...
                    <div class="section-block mb-5">
                        ...
                        <div class="row g-4">
                            <div class="col-md-4">
                                ...
                                <input type="text" id="supplier_id" name="supplier_id" class="form-control" required readonly>
                                @error('supplier_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const supplier_idField = document.getElementById('supplier_id'); // ubah nama biar sesuai

        // Pastikan elemen ada dulu
        if (!supplier_idField) return;

        fetch('{{ route('suppliers.generateSupplierId') }}', {
            headers: {
        ...
        .then(data => {
            if (data.supplier_id) {
                supplier_idField.value = data.supplier_id; // variabel sekarang valid
            }
        })
        .catch(error => console.error('❌ Error fetching supplier_id:', error));
    });
</script>
